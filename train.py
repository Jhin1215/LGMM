...
def fit(args):
    ...
    if args.resume is not None and args.resume:
        ...
        # 加载上一次训练过程中, 模型最高的 f1 分数作为当前已经训练出最好的模型
    ...
    for epoch in range(args.start_epoch, args.total_epoch):
        ...
        # _ = train_with_amp(args, dataloader_train, model, optimizer, loss_fn,
        #                    epoch, cur_iter, logger, timer, lr_scheduler)
        cur_iter += len(dataloader_train)

        f1_val, f1_loss = val(args, dataloader_val, model, loss_fn, optimizer.param_groups[0]["lr"], epoch, logger)
