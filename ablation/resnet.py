...
class AdaptedResNet18(nn.Module):
    def __init__(self, pre_trained=False, output_channels=[64, 96, 128, 256]):
        super().__init__()
        ...
        self.channel_adapters = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(64, output_channels[0], kernel_size=1, bias=False),
                nn.BatchNorm2d(output_channels[0]),
                nn.ReLU(inplace=True)
            ),
            nn.Sequential(
                nn.Conv2d(128, output_channels[1], kernel_size=1, bias=False),
                nn.BatchNorm2d(output_channels[1]),
                nn.ReLU(inplace=True)
            ),
            nn.Sequential(
                nn.Conv2d(256, output_channels[2], kernel_size=1, bias=False),
                nn.BatchNorm2d(output_channels[2]),
                nn.ReLU(inplace=True)
            ),
            nn.Sequential(
                nn.Conv2d(512, output_channels[3], kernel_size=1, bias=False),
    ...
    def forward(self, x):
        ...
        features.append(self.channel_adapters[0](x1))  # 调整到64通道

        x2 = self.backbone.layer2(x1)
        features.append(self.channel_adapters[1](x2))  # 调整到96通道

        x3 = self.backbone.layer3(x2)
        features.append(self.channel_adapters[2](x3))  # 调整到128通道

        x4 = self.backbone.layer4(x3)
        features.append(self.channel_adapters[3](x4))  # 调整到256通道

        return features  # 返回调整后的特征列表
